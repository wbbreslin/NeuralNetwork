...
# Compute gradients
grad_loss_w0 = torch.autograd.grad(loss, w0, create_graph=True)[0]

# Create a vector for Hessian-vector product
vector_v = torch.tensor([[0,0],[1,0],[0,0]], requires_grad=False, dtype=torch.float64)

# Compute Hessian-vector product manually
hessian_vector_product = torch.autograd.grad(grad_loss_w0, w0, vector_v, retain_graph=True)

print("Hessian-vector product:", hessian_vector_product)

#This  matches the first diagonal block