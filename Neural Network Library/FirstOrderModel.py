...
def first_order_model(x_predictors, y_outcomes, weights):
    ...
    """
    states, first_derivatives = forward_pass(x_predictors,
                                             weights)
...
def first_order_model2(x_predictors, y_outcomes, weights):
    """
    Description
    --------------------
    This is a wrapper function combining the forward pass and backward passes into one function.
    ...
    Lambdas             : List of adjoint variable states at each layer of the NN
    """
    states, augmented_states, first_derivatives = forward_pass2(x_predictors,
                                             weights)

    Lambdas, gradients = backward_pass2(states,
                                       y_outcomes,
                                       first_derivatives,
                                       weights)
    y_predictions = states[-1]
...
def forward_pass2(x_predictors, weights):
    """
    Description
    --------------------
    Neural Network model dynamics (Forward Pass)
...
def backward_pass(states, y_outcomes, first_derivatives, weights):
    ...
    for i in reversed(range(len(weights))):
        gradient = np.kron(np.eye(p),states[i]).T @ first_derivatives[i] @ Lambda
        new_Lambda = np.kron(weights[i].T,np.eye(n)).T @ first_derivatives[i] @ Lambda
...
def backward_pass2(states, y_outcomes, first_derivatives, weights):
    """
    Description
    --------------------
    First-order adjoint model (Backward Pass)
